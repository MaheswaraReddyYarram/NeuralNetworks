...
# initialize output path and hyper parameters
CUDA = False
...
DCGAN.utils.clear_folder(OUT_PATH)
print("Logging to file:{}\n".format(LOG_FILE))
sys.stdout = DCGAN.utils.StdOut(LOG_FILE)
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCH_NUM):
        for i, data in enumerate(dataloader):
            ...
            noise = torch.randn(x_real.size(0), Z_DIM, 1, 1, device=device)
            ...
            if i % 100 == 0:
                ...
                with torch.no_grad():
                    ...
                    vutils.save_image(viz_sample, os.path.join(OUT_PATH, 'fake_samples_{}.png'.format(epoch)),
                                      normalize=True)
            torch.save(generator.state_dict(), os.path.join(OUT_PATH, 'netG_{}.pth'.format(epoch)))
            torch.save(discriminator.state_dict(), os.path.join(OUT_PATH, 'netD_{}.pth'.format(epoch)))
