...
class Generator(nn.Module):
    def __init__(self, channels, latent_dim=20, embed_dim = 1024, embed_out_dim=128):
    ...
    def _create_layer(self, size_in, size_out, kernel_size = 4, stride = 2, padding = 1, output = False):
        layers = [nn.ConvTranspose2d(size_in, size_out, kernel_size, stride, padding, bias=False)]
        if output:
...
class Discriminator(nn.Module):
    def __init__(self, channels, embed_dim=1024, embed_out_dim=128):
        ...
        self.model = nn.Sequential(
            *self._create_layer(self.channels, 64, 4, stride=2, padding=1, normalize=False),
    ...
    def _create_layer(self, size_in, size_out, kernel_size = 4, stride = 2, padding = 1, normalize=True):
        layers = [nn.Conv2d(size_in, size_out, kernel_size, stride, padding)]
        if normalize:
