...
class Model(object):
    ...
    def create_optim(self,lr, alpha=0.5, beta=0.99):
        self.optimG = optim.Adam(self.netG.parameters(), lr=lr, betas=(alpha, beta))
        self.optimD = optim.Adam(self.netD.parameters(), lr=lr, betas=(alpha, beta))

    def train(self, epochs, log_interval =100, out_dir='', verbose=True):
        self.netG.train()
        self.netD.train()
        total_time = datetime.time.time()
        for epoch in range(epochs):
            batch_time = datetime.time.time()
            for batch_idx, data in enumerate(self.data_loader):
                image =  data['right_images'].to(self.device)
                embed = data['right_embed'].to(self.device)

                real_label = torch.ones(image.shape[0], 1).to(self.device)
                fake_label = torch.zeros(image.shape[0], 1).to(self.device)
                ...
                noise =  torch.randn(image.shape[0], 100, 1, 1).to(self.device)
                ...
                g_loss = self.loss_adv(out_fake, real_label) + self.l1_coef * l1_loss + self.l2_coef * self.loss_l2(image_fake, image)
                g_loss.backward()
                self.optimG.step()
    ...
    def eval(self, batch_size=None):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.data_loader):
                ...
                vutils.save_image(viz_sample, 'img_{}.png'.format(batch_idx), nrow=batch_size // 8, normalize=True)
                for t in text:
                    print(t)
                break
